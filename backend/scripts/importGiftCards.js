...
const loadGiftCards = () => {
  try {
    const filePath = path.join(__dirname, '../../scraper/data/gift_cards.json');
    console.log('Loading gift cards from:', filePath);
    const data = fs.readFileSync(filePath, 'utf8');
    const items = JSON.parse(data);
    console.log(`Successfully loaded ${items.length} gift cards`);
    return items;
  } catch (err) {
    console.error('Error reading gift cards file:', err);
    process.exit(1);
  }
};

const transformGiftCard = (item) => {
  const price = {
  ...
  return {
    title: item.title || 'Untitled Gift Card',
    slug: item.title
    ...
    stockQuantity,
    url: item.url,
    type: item.type || 'Gift Card',
    scrapedAt: item.scrapedAt ? new Date(item.scrapedAt) : new Date(),
    lastUpdated: new Date()
  };
};

const importGiftCards = async () => {
  try {
    await connectDB();

    const rawItems = loadGiftCards();
    const items = rawItems.map(transformGiftCard).filter(i => i.url);

    if (!items.length) {
      console.log('No gift cards found in source data.');
      process.exit(0);
    }
    ...
    console.log(`Upserting ${bulkOps.length} gift cards...`);
    const result = await GiftCard.bulkWrite(bulkOps);
    console.log(
      `Gift cards upsert complete. Inserted: ${result.upsertedCount}, Modified: ${result.modifiedCount}`
    );

    const uniqueUrls = [...new Set(urls)];
    const staleResult = await GiftCard.updateMany(
      { url: { $nin: uniqueUrls } },
      { $set: { isInStock: false, stockQuantity: 0 } }
    );
    console.log(
      `Marked ${staleResult.modifiedCount} existing gift cards as out of stock (not in latest import).`
    );
