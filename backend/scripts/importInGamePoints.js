...
const InGamePoint = require('../src/models/InGamePoint');
...
const loadInGamePoints = () => {
  try {
    const filePath = path.join(__dirname, '../../scraper/data/in_game_points.json');
    console.log('Loading in-game points from:', filePath);
    const data = fs.readFileSync(filePath, 'utf8');
    const items = JSON.parse(data);
    console.log(`Successfully loaded ${items.length} in-game points products`);
    return items;
  } catch (err) {
    console.error('Error reading in-game points file:', err);
    process.exit(1);
  }
};

const transformInGamePoint = (item) => {
  const price = {
  ...
  return {
    ...
    stockQuantity,
    url: item.url,
    type: item.type || 'In-Game Points',
    scrapedAt: item.scrapedAt ? new Date(item.scrapedAt) : new Date(),
    lastUpdated: new Date()
  };
};

const importInGamePoints = async () => {
  try {
    await connectDB();

    const rawItems = loadInGamePoints();
    const items = rawItems.map(transformInGamePoint).filter(i => i.url);

    if (!items.length) {
      console.log('No in-game points products found in source data.');
      process.exit(0);
    }
    ...
    console.log(`Upserting ${bulkOps.length} in-game points products...`);
    const result = await InGamePoint.bulkWrite(bulkOps);
    console.log(
      `In-game points upsert complete. Inserted: ${result.upsertedCount}, Modified: ${result.modifiedCount}`
    );

    const uniqueUrls = [...new Set(urls)];
    const staleResult = await InGamePoint.updateMany(
      { url: { $nin: uniqueUrls } },
      { $set: { isInStock: false, stockQuantity: 0 } }
    );
    console.log(
      `Marked ${staleResult.modifiedCount} existing in-game points products as out of stock (not in latest import).`
    );

    process.exit(0);
  } catch (err) {
    console.error('Error importing in-game points:', err);
    process.exit(1);
  }
};
