...
class SubscriptionScraper {
  constructor(maxPages = 10) {
    ...
    this.subscriptionUrl = `${this.baseUrl}/product-category/subscriptions`;
      ...
      $('.woocommerce-product-attributes-item').each((i, el) => {
        ...
        if (label && value) {
          ...
          if (label.toLowerCase().includes('duration')) {
            product.subscriptionDetails.duration = value;
          }
          if (label.toLowerCase().includes('platform')) {
            product.subscriptionDetails.platform = value;
          }
          if (label.toLowerCase().includes('region')) {
            product.subscriptionDetails.region = value;
          }
        }
      });
  ...
  async scrapeProductsList(page = 1) {
    try {
      ...
      const url = page === 1 
        ...
        : `${this.subscriptionUrl}/page/${page}/`;
  ...
  async saveToFile(data, filename = 'subscriptions.json') {
    try {
      ...
      const backupPath = path.join(this.dataDir, `subscriptions_${timestamp}.json`);
  ...
  async run() {
    try {
      console.log('🚀 Starting Subscription Scraper...');
      console.log(`🌐 Base URL: ${this.subscriptionUrl}`);
      ...
      console.error('Error in SubscriptionScraper:', error);
      throw error;
    }
  }
}
