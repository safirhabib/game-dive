...
class InGamePointsScraper {
  constructor(maxPages = 10) {
    this.baseUrl = 'https://gamecastlebd.com';
    this.inGamePointsUrl = `${this.baseUrl}/product-category/in-game-points`;
  ...
  async scrapeProductsList(page = 1) {
    try {
      ...
      const url = page === 1 
        ? this.inGamePointsUrl 
        : `${this.inGamePointsUrl}/page/${page}/`;
  ...
  async saveToFile(data, filename = 'in_game_points.json') {
    try {
      ...
      const backupPath = path.join(this.dataDir, `in_game_points_${timestamp}.json`);
  ...
  async run() {
    try {
      ...
      console.log(`🌐 Base URL: ${this.inGamePointsUrl}`);
      ...
      console.error('Error in InGamePointsScraper:', error);
      throw error;
    }
  }
}

module.exports = InGamePointsScraper;
