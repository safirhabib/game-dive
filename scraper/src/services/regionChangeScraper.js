...
class RegionChangeScraper {
  constructor(maxPages = 10) {
    this.baseUrl = 'https://gamecastlebd.com';
    this.regionChangeUrl = `${this.baseUrl}/product-category/games/steam-epic-region-change`;
  ...
  async scrapeProductsList(page = 1) {
    try {
      ...
      const url = page === 1 
        ? this.regionChangeUrl 
        : `${this.regionChangeUrl}/page/${page}/`;
  ...
  async saveToFile(data, filename = 'region_change_games.json') {
    try {
      ...
      const backupPath = path.join(this.dataDir, `region_change_games_${timestamp}.json`);
  ...
  async run() {
    try {
      console.log('🚀 Starting Steam/Epic Region Change Scraper...');
      console.log(`🌐 Base URL: ${this.regionChangeUrl}`);
      ...
      console.error('Error in RegionChangeScraper:', error);
      throw error;
    }
  }
}

module.exports = RegionChangeScraper;
